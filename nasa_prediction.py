...
# --- Streamlit UI Setup ---
st.title("🚀 Exoplanet Detection using NASA Kepler Data")
st.markdown("""
...
# --- Custom Prediction Input (All Features) ---
st.subheader("🧮 Try a Custom Prediction")

st.write("Enter values for all features to predict whether it's an exoplanet:")

sample = []
for col in X.columns:  # Use ALL columns the model was trained on
    val = st.number_input(f"{col}", value=float(X[col].mean()))
    sample.append(val)

if st.button("🔍 Predict Exoplanet?"):
    prediction = model.predict([sample])[0]  # Now the length matches model input
    if prediction == 1:
        st.success("✅ This object is likely an **Exoplanet!** 🌍")
    else:
        st.error("❌ This object is likely **Not a Planet.**")

st.caption("Developed by Deepan Sanjeev | Powered by Python, Scikit-learn & Streamlit")


st.caption ("Turning Kepler data into cosmic discoveries! ✨")